...
def main():
    ...
    **Status Criteria (Tolerance: ±{tolerance}%)**
    ...
    with tab1:
        ...
        # 3D Bar Chart - Top 15 Parts by Stock Value
        st.subheader("📊 3D Bar Chart: Top 15 Parts by Stock Value")
        
        # Get top 15 parts by stock value
        top_15_parts = sorted(processed_data, key=lambda x: x['Stock_Value'], reverse=True)[:15]
        
        # Prepare data for 3D bar chart
        materials = [item['Material'] for item in top_15_parts]
        descriptions = [item['Description'] for item in top_15_parts]
        qtys = [item['QTY'] for item in top_15_parts]
        rms = [item['RM IN QTY'] for item in top_15_parts]
        stock_values = [item['Stock_Value'] for item in top_15_parts]
        statuses = [item['Status'] for item in top_15_parts]
        vendors = [item['Vendor'] for item in top_15_parts]
        variances = [item['Variance_%'] for item in top_15_parts]
        
        # Create 3D bar chart
        fig_3d_bar = go.Figure()
        
        # Add bars for Current QTY
        fig_3d_bar.add_trace(go.Scatter3d(
            ...
            name='Current QTY',
            hovertemplate=(
                '<b>Part No:</b> %{customdata[0]}<br>'
                '<b>Description:</b> %{customdata[1]}<br>'
                '<b>Vendor:</b> %{customdata[5]}<br>'
                '<b>Current QTY:</b> %{z}<br>'
                '<b>Stock Value:</b> ₹%{customdata[3]:,}<br>'
                '<b>Status:</b> %{customdata[4]}<br>'
                '<b>Variance:</b> %{customdata[6]:.1f}%'
                '<extra></extra>'
            ),
            customdata=list(zip(materials, descriptions, rms, stock_values, statuses, vendors, variances))
        ))
        ...
        fig_3d_bar.add_trace(go.Scatter3d(
            ...
            hovertemplate=(
                ...
                '<b>Description:</b> %{customdata[1]}<br>'
                '<b>Vendor:</b> %{customdata[5]}<br>'
                '<b>Required QTY:</b> %{z}<br>'
                '<b>Stock Value:</b> ₹%{customdata[3]:,}<br>'
                '<b>Status:</b> %{customdata[4]}<br>'
                '<b>Variance:</b> %{customdata[6]:.1f}%'
                '<extra></extra>'
            ),
            customdata=list(zip(materials, descriptions, qtys, stock_values, statuses, vendors, variances))
        ))
        
        # Add connecting lines between QTY and RM
        for i in range(len(materials)):
            fig_3d_bar.add_trace(go.Scatter3d(
                ...
                mode='lines',
                line=dict(
                    color=analyzer.status_colors[statuses[i]],
                    width=4
                ),
                showlegend=False,
                hoverinfo='skip'
        ...
        fig_3d_bar.update_layout(
            title="3D Comparison: Current QTY vs Required QTY (Top 15 by Stock Value)",
            scene=dict(
                xaxis=dict(
                    title='Parts (Index)',
                    tickmode='array',
                    tickvals=list(range(len(materials))),
                    ticktext=[f"{mat[:8]}..." if len(mat) > 8 else mat for mat in materials]
                ),
                yaxis=dict(
                    title='Quantity Type',
                    tickmode='array',
                    tickvals=[0, 1],
                    ticktext=['Current', 'Required']
                ),
        ...
        # 3D Surface Plot - Vendor Performance Analysis
        st.subheader("📊 3D Surface: Vendor Performance Landscape")
        
        # Prepare vendor performance data for surface plot
        vendor_perf_data = []
        for vendor, data in vendor_summary.items():
            total_parts = data['total_parts']
            short_percentage = (data['short_parts'] / total_parts) * 100 if total_parts > 0 else 0
            excess_percentage = (data['excess_parts'] / total_parts) * 100 if total_parts > 0 else 0
            avg_stock_value = data['total_value'] / total_parts if total_parts > 0 else 0
            
            vendor_perf_data.append({
                'vendor': vendor,
                'short_pct': short_percentage,
                'excess_pct': excess_percentage,
                'avg_value': avg_stock_value,
                'total_parts': total_parts,
                'total_value': data['total_value']
            })
        
        if len(vendor_perf_data) >= 3:  # Need at least 3 points for surface
            # Create meshgrid for surface plot
            x = [item['short_pct'] for item in vendor_perf_data]
            y = [item['excess_pct'] for item in vendor_perf_data]
            z = [item['avg_stock_value'] for item in vendor_perf_data]
            
            # Create hover text for surface plot
            surface_hover = []
            for item in vendor_perf_data:
                hover_info = (
                    f"<b>Vendor:</b> {item['vendor']}<br>"
                    f"<b>Short Inventory:</b> {item['short_pct']:.1f}%<br>"
                    f"<b>Excess Inventory:</b> {item['excess_pct']:.1f}%<br>"
                    f"<b>Avg Stock Value:</b> ₹{item['avg_value']:,.0f}<br>"
                    f"<b>Total Parts:</b> {item['total_parts']}<br>"
                    f"<b>Total Value:</b> ₹{item['total_value']:,}"
                )
                surface_hover.append(hover_info)
            
            fig_3d_surface = go.Figure(data=go.Scatter3d(
                x=x,
                y=y,
                z=z,
                mode='markers+text',
                marker=dict(
                    ...
                    colorscale='Viridis',
                    showscale=True,
                    colorbar=dict(title="Avg Stock Value (₹)")
                ),
                text=[item['vendor'] for item in vendor_perf_data],
                textposition="top center",
                hovertemplate='%{customdata}<extra></extra>',
                customdata=surface_hover
            ))
